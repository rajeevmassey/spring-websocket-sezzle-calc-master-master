	...
	.controller('ChatController', ['$scope', '$location', '$interval', 'toaster', 'ChatSocket', function($scope, $location, $interval, toaster, chatSocket) {
		...
		$scope.sendMessage = function() {
			...
			if($scope.sendTo != "everyone") {
				...
				$scope.messages.unshift({expression: $scope.newMessage, username: 'you', priv: true, to: $scope.sendTo});
			}
			
			chatSocket.send(destination, {}, JSON.stringify({expression: $scope.newMessage}));
			$scope.newMessage = '';
		};
